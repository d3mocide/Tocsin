...
| `TOCSIN_DATA_DIR` | *(unset)* | Directory holding `same_event_codes.yaml`, `fips.csv`, and `nwr_stations/*.yaml` for `GET /reference`. Unset or missing degrades to an empty reference rather than refusing to start -- unlike every other service, where a missing tier table would mean mis-tiering a real warning. |
| `TOCSIN_LATITUDE` / `TOCSIN_LONGITUDE` | *(unset)* | Operator's approximate location, decimal degrees. Both set adds `distance_km`/`distance_miles` to every station in `GET /reference`'s `stations` table (haversine, `reference.py`) so the UI can list nearby NWR transmitters; unset leaves both `null` for all of them rather than a wrong or fabricated number. `reference.py` merges every file under `data/nwr_stations/` (one per state); two stations (WZ2522, WZ2559) have no coordinates of their own yet and stay `null` regardless. |
