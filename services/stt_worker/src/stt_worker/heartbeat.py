"""Liveness heartbeat: stt_worker SETEXes `tocsin:status:stt_worker` from its
...
crashed stt_worker and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"stt_worker: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
