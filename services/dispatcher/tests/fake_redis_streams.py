...
consumer-group API `redis_bus.py` actually uses. Deliberately duplicated
from `fusion`'s own test fake rather than imported -- test code respects
the same service boundary as source (CLAUDE.md). See fusion's copy for the
full reasoning on why `">"`/`"0"` semantics matter here.
"""
