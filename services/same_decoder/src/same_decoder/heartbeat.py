"""Liveness heartbeat: same_decoder SETEXes `tocsin:status:same_decoder` from its
...
crashed same_decoder and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"same_decoder: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
