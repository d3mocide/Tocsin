"""Liveness heartbeat: nws_poller SETEXes `tocsin:status:nws_poller` from its
...
crashed nws_poller and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"nws_poller: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
