"""Liveness heartbeat: live_audio SETEXes `tocsin:status:live_audio` from its
...
crashed live_audio and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"live_audio: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
