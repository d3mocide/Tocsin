...
| `LIVE_AUDIO_CHANNELS` | *(unset -- all channels)* | Comma-separated NWR channel allowlist, e.g. `WX5,WX7`. Gates which channels ever get a feeder (`service.py`'s `Streamer`) -- a channel not in the list never spawns an ffmpeg process or opens an Icecast source connection, rather than streaming silence. Most sites only have usable signal on one or two of the seven channels; the rest otherwise ran a permanent encode/connection for no listener (`docs/design/tracking.md`). SAME decode and the alert ring buffer are sdr-rx's, not this service's, and keep watching every channel regardless -- this only trims the audible feed. |
