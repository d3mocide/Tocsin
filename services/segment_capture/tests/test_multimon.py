...
from segment_capture.multimon import MultimonProcess
...
def test_multiple_lines_are_all_delivered_in_order():
    process = MultimonProcess(command=_fake_command("EAS: ZCZC-WXR-TOR-017021+0045-1000042-KILX/NWS-", "EAS: NNNN"))
    process.write(b"\x00\x00\x00\x00")
    lines = _wait_for_lines(process, 2)
    assert lines == ["EAS: ZCZC-WXR-TOR-017021+0045-1000042-KILX/NWS-", "EAS: NNNN"]
    process.close()
