...
| `LIVE_TRANSCRIPTION_RMS_THRESHOLD` | `0.02` | Speech/silence threshold on raw discriminator output. **Uncalibrated** (master prompt §12) -- varies with gain, squelch, and transmitter. Set too high it produces silence forever with no other symptom, which is why the status line below exists. |
| `LIVE_TRANSCRIPTION_MIN_CHUNK_SECONDS` / `_MAX_CHUNK_SECONDS` / `_SILENCE_HANG_SECONDS` | `3` / `20` / `1` | Shortest chunk worth cutting, hard cap before cutting regardless of silence, and how much silence ends a chunk. |
| `LIVE_TRANSCRIPTION_STATUS_INTERVAL_SECONDS` | `300` | How often to log `live <site>/<channel> last Ns: rms mean ... peak ... vs threshold ..., N% of audio counted as speech, N chunk(s) sent`. Tune the threshold from this line; it also names the fix outright when nothing clears the threshold. |
| `LIVE_TRANSCRIPTION_SITE` / `LIVE_TRANSCRIPTION_CHANNEL` | *(unset)* | The single `(site, channel)` to transcribe continuously. Both required if enabled -- logs a warning and stays disabled otherwise. `SITE` is the site *name* from `SDR_RX_DEVICES=site:serial` (e.g. `PDX`), not the dongle serial; pasting a whole `PDX:49435794` entry also works, since the site half is taken from it. A site with no ring buffer logs which sites/channels *do* exist and keeps retrying rather than crashing -- this process also runs the core SAME-alert capture path, which must never go down over a live-transcription misconfiguration. |
