"""Liveness heartbeat: segment_capture SETEXes `tocsin:status:segment_capture` from its
...
crashed segment_capture and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"segment_capture: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
