"""Liveness heartbeat: fusion SETEXes `tocsin:status:fusion` from its
...
crashed fusion and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"fusion: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
