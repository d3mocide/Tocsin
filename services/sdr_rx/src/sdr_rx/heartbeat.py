"""Liveness heartbeat: sdr_rx SETEXes `tocsin:status:sdr_rx` from its
...
crashed sdr_rx and a working one produce identical output, and a status
...
class Heartbeat:
    ...
    def beat(self, **detail) -> bool:
        ...
        try:
            ...
            print(f"sdr_rx: heartbeat write failed: {exc}", file=sys.stderr)
            return False
        return True
